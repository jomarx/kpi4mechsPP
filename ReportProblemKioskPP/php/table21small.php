...
$dbname = "kpi_mech";
$style3 = "<td style = 'wlocationth: 150px; border: 4px solid black; background-color: #ffff00; font-size: 20px; ' align='center'>";//format if StartTime exceed $limit in minutes
$style1 = "<td style = 'wlocationth: 150px; border: 4px solid black; background-color: #ff6666; font-size: 20px; ' align='center'>";//format if StartTime exceed $limit in minutes

$style2 = "<td style = 'wlocationth: 150px; border: 4px solid black; background-color: #ffffff; font-size: 20px; ' align='center'>";//format otherwise
$limit = 15;
$limit2 = 30;
...
$sqlstatus5 = "SELECT ID, SpecialSerial FROM kpi_mech.task_db WHERE Status = '5' limit 1 ";
$resultstatus5 = $conn->query($sqlstatus5);
	 if ($resultstatus5->num_rows > 0) {
		...
		$idstatus5 = $rowstatus5["ID"];
					...
					$sqlmechcanceled = "SELECT mech FROM kpi_mech.cancel_db WHERE taskID = $idstatus5";
...
$sqlstatus6 = "SELECT ID, SpecialSerial FROM kpi_mech.task_db WHERE Status = '6' limit 1 ";
$resultstatus6 = $conn->query($sqlstatus6);
	 if ($resultstatus6->num_rows > 0) {
		...
		$idstatus6 = $rowstatus6["ID"];
					...
					$sqlmechtimeout = "SELECT mech FROM kpi_mech.cancel_db WHERE taskID = $idstatus6";
									...
									$resultselectnot2 = $conn->query($sqlselectnot2);
										...
										if ($resultselectnot2->num_rows > 0) {
											...
											$selectnot2=$rowselectnot2["NotifNo"];
														...
														$sqlupdatetimeout = "UPDATE kpi_mech.task_db SET Location ='1',Status='1',NotifNo=$selectnot2,Assignee=$mechtimeout WHERE ID = $idstatus6" ;
